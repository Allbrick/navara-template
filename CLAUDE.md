...
17. **모델마다 애니메이션 클립 이름이 다릅니다.** Fox는 `Survey`/`Walk`/`Run`,
    ...
    **메시가 모델 원점에서 벗어나 있으면 캐릭터가 화면 한쪽으로 밀려 보입니다.** 카메라는
    ...
    `translation: [1.3, 0, 0]`이 박혀 있습니다.

    이때 **Descriptor의 `position`으로 상쇄하면 안 됩니다.** 플러그인이 매 프레임 다시
    쓰는 변환과 충돌해 캐릭터가 한 방향으로 계속 흘러갑니다. `recenterNodes`로 **모델
    안쪽 노드의 지역 translation을 직접 0으로** 만드세요.

    **모델 손질은 찾을 때까지 재시도해야 합니다.** `model.ref.raw`가 생긴 시점에도 하위
    트리는 아직 비어 있을 수 있어서, raw가 있으면 바로 끝내는 구현은 아무것도 숨기지
    못합니다(human.glb의 바닥 평면이 계속 보였던 원인). 캐릭터 전환 시 모델이 새로
    로드되므로 성공 후에도 잠시 더 지켜보며 다시 적용합니다 — `applyModelTweaks` 참고.

    크기는 두 모델 모두 미터 단위라 `modelScale: 1`입니다.

    **표준 Y-up glTF는 전부 `modelRotationOffset` 보정이 필요합니다.** 캐릭터 프레임이
    ENU(+Z가 위)라, 보정 없이 두면 모델의 앞(+Z)이 하늘을 향해 **얼굴을 위로 하고 등으로
    눕습니다.** `{ x: π/2, y: π, z: 0 }`이 정답입니다 — `Rx(90°)`가 세우고, 그러면 앞이
    남쪽을 보므로 `Ry(180°)`로 돌려 heading 0(정북)과 맞춥니다. 세 모델 모두 같은 값입니다.

    디버깅 주의: 이 오프셋은 **`model.ref.raw.rotation`에 반영되지 않습니다**(월드 행렬로
    들어갑니다). 그 값이 0이라고 보정이 안 걸린 것으로 오해하지 마세요. 반대로 그 값을
    직접 바꿔 실험하면 `setHeading` 등이 다시 덮어씁니다.

    마우스 드래그로 시점을 돌리려면 `allowCameraControl: true`가 필요합니다. 기본값은
    false라 **Alt를 누른 채로만** 카메라가 자유로워집니다.
